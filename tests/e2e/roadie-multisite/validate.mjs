...
try {
  ...
  for (const [slug, pluginFile] of Object.entries(componentFiles)) {
    ...
    await writeFile(path.join(componentPath, pluginFile), `<?php\n/* Plugin Name: ${slug} validation fixture */\n`);
    run('git', ['init', '--quiet', componentPath]);
    run('git', ['-C', componentPath, 'add', pluginFile]);
  ...
  const settings = buildSettings(components, '7.0');

  assert.equal(settings.wp_codebox_extra_plugins.length, Object.keys(componentFiles).length + 1);
  assert.equal(settings.wordpress_runtime_prepare_steps[0].metadata.roadie_e2e_provenance.components.length, Object.keys(componentFiles).length);
  assert.deepEqual(
